...
class NotificationTest {
    ...
    @Before fun setUp() {
        ...
        Bildirim.initialize(ctx, "pk_test", BildirimConfig(apiBase = api.baseUrl, tokenProvider = FakeTokenProvider("tok-1")))
    ...
    @Test fun `onMessageReceived yabanci mesaja dokunmaz, bizimkini cizer ve displayed gonderir`() {
        assertEquals(false, Bildirim.onMessageReceived(mapOf("x" to "y")))
        assertEquals(true, Bildirim.onMessageReceived(mapOf("bildirim" to raw)))
        Bildirim.runtimeForTests()!!.sync.awaitIdle()
        val nm = ctx.getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager
    ...
    @Test fun `tiklama - clicked olayi, actionId ve adres acma`() {
        ...
        var opened: BildirimNotification? = null
        ...
        assertEquals("kamp1", opened!!.campaignId)
        assertEquals("Başlık", opened!!.title)
